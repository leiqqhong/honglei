/**
 * Created with jing.zhao2013 Date: 13-7-30 Time: 下午4:03
 */
...
$(function() {
	...
	$("#userId").blur(
			function() {
				var userId = $(this).val();
				$.getJSON("EasybuyUserServlet?opr=checkUser&userId=" + userId,
						function() {
							if (ret) {
								$("#userIdSpan").html("用户名已经存在");
								$("#userIdSpan").addClass("error");
							}
						});
			});
	// 表单提交事件
	...
	$("#addr")
			.click(
					function() {
						...
						if (val == "添加") {
							// location.href =
							// "Cart?action=addAddress&address="+$("#addAddr").val();
	...
	$(".delete")
			...
			.click(
					function() {
						...
						var epId = tr.attr("id");// 获得商品Id
						if (confirm("确定要删除吗？")) {
							location.href = "EasybuyProductServlet?action=shoppingCart&opr=del&epId="
									+ epId;
						} else {
							return false;
						}
	...
	$(".number")
			...
			.click(
					...
					/*	alert(num);*/
						location.href = "EasybuyProductServlet?action=shoppingCart&opr=update&epId="
								+ epId + "&num=" + num;
						/*
						 * var $tds=$(this).parent().parent().children("td");
						 * var $price= $($tds[1]); var $number=$($tds[2]); var
						 * price =
						 * $price.find("input[type='hidden']").val();//存值 var
						 * $priceBox =$price.find("span");//现实价钱 var $number=
						 * $number.find("input");//得到存储input对象 var
						 * opr=$(this).attr("name");//判断增减 var number =
						 * $number.val();//数量 if(opr=="del"){ number--;
						 * if(number<=0){ if(confirm("确定要删除吗？")) {
						 * $price.parent().remove(); }else{ number=1; } } }else
						 * if(opr=="add"){ number++; } $number.val(number);
	...
	/*function totalPrice() {
		 ...
		 * var p= parseFloat($(d).val()); var n =
		 * $(d).parent().parent().find("input[name='number']").val();
		 * totalPrice=totalPrice+p*n; }); return totalPrice;
		 
	}*/
//	$("#shopping").find("input[name='number']").change(function() {
//		/*
//		 * var v=$(this).val(); if(!(/^[0-9]*[1-9][0-9]*$/.test(v))){
//		 * alert("请输入正整数"); $(this).val(1); } var
//		 * $price=$($(this).parent().parent().children("td")[1]); var p =
